...
.wh_content_item div .wh_isMark::after {
      ...
      <div class="wh_content">
        <div class="wh_content_item" v-for="(item,index) in list" @click="clickDay(item,index)">
          <div>
            ...
            <li v-else="(isHideOtherday&&item.nextDayShow)||item.otherMonth||item.dayHide" v-bind:class="{ wh_isToday: item.isToday,wh_isMark:item.isMark,isTodayNow:item.isTodayNow}" :data-mark-text='item.markText'>
              {{item.id}}
            </li>
          </div>
        </div>
...
<script>
//基于https://github.com/zwhGithub/vue-calendar
//修改添加了显示markText功能
      ...
      for (var i = 0; i < this.getDaysInOneMonth(date); i++) {
        ...
        if (
          ...
          array = array.concat({
            ...
            markText: this.markTextArray[this.markArray.indexOf(i + 1)],
            dayHide:
      ...
      for (var i = 0; i < list.length; i++) {
        ...
        if (list[i].date) {
          for (var n = 0; n < val.length; n++) {
            if (list[i].id == val[n]) {
              list[i].isMark = true
              list[i].markText = this.markTextArray[n]
            }
          }
        }
      }
